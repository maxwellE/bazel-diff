...
# the guest is self-contained and offline: a JDK (for the Bazel server), a pinned
# `bazel` binary, git, the statically linked bazel-diff binary (at
...
# The musl static build for $ARCH (`bazel build //release:bazel-diff
# --config=release-musl` or `--config=release-musl-arm64`), so it runs in the
# guest regardless of which glibc the base image ships.
BAZEL_DIFF_BIN="${BAZEL_DIFF_BIN:?set BAZEL_DIFF_BIN to the statically linked bazel-diff binary for $ARCH}"
