...
The e2e suite in this repo used to be one target running dozens of cases in a
single process. That made a case's runtime invisible (one timeout covered the
lot), its failures expensive (a re-run re-ran everything) and its result
uncacheable independently. This macro gives every case its own target instead,
with its own timeout -- `moderate`, i.e. 300 seconds, unless the case's source
declares otherwise. //tools/e2e:split_e2e_tests.py explains why that default is
not the tighter `short`.
...
# host's core count and cheerfully tries to use all of it. Left untagged, Bazel
# schedules as many cases at once as the runner has cores, and each of those
# spawns a nested build that thinks it owns the machine. Measured on a 28-core
# Mac, a suite run that way turns a 17s case into a 142s one and pushes the two
# `serve` cases into failing outright.
#
...
# parallelism along with it -- which is why the suite, tagged `exclusive`, took
# 640s split against 250s in a single process. Reserving 4 CPUs lets a 28-core
# machine run 7 cases at once and a 4-core CI runner fall back to one at a time,
# without either having to name a concurrency limit anywhere.
#
...
def rust_e2e_tests(suites, args = [], tags = [], case_cpus = _CASE_CPUS, **kwargs):
    ...
    leave a target that passes without running anything. //tools/e2e:regen_check
    is what rules that out: it fails the build the moment a case name in the
    generated list stops matching the sources.
