...
per-runner tracefiles (rules_rust's llvm-cov toolchain exports LCOV,
rules_go converts Go cover profiles to LCOV) into the `coverage.dat` that
Bazel publishes for the test. Two properties
...
- `coverage_include` — optional path prefixes scoping which source files
  ...
  a focused test's percentage is not diluted by files it never exercises.
